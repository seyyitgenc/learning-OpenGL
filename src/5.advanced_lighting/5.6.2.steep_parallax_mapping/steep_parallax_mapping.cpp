...
float height_scale = 0.0;

bool isOcclusionEnabled = false;
bool isOcclusionPressed = false;

bool isLayersDiffer = false;
bool isLayersDifferPressed = false;
...
int main()
{
    ...
    // shaders
    Shader ourShader(FileSystem::getPath("bin/shaders/5.advanced_lighting/parallax_mapping_steep.vs").c_str(),
                     FileSystem::getPath("bin/shaders/5.advanced_lighting/parallax_mapping_steep.fs").c_str());

    Shader lightCubeShader(FileSystem::getPath("bin/shaders/2.lighting/light_cube.vs").c_str(),
                           FileSystem::getPath("bin/shaders/2.lighting/light_cube.fs").c_str());

    unsigned int diffuseMap = loadTexture(FileSystem::getPath("bin/resources/textures/toy_box_diffuse.png").c_str());
    unsigned int normalMap = loadTexture(FileSystem::getPath("bin/resources/textures/toy_box_normal.png").c_str());
    unsigned int depthMap = loadTexture(FileSystem::getPath("bin/resources/textures/toy_box_disp.png").c_str());
    glm::vec3 lightPos(0.8f, 0.0f, 0.3);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ourShader.setBool("isOcclusionEnabled", isOcclusionEnabled);
        ourShader.setBool("isLayersDiffer", isLayersDiffer);
...
void processInput(GLFWwindow *window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_H) == GLFW_PRESS && !isOcclusionPressed)
    {
        isOcclusionEnabled = !isOcclusionEnabled;
        isOcclusionPressed = true;
    }
    if (glfwGetKey(window, GLFW_KEY_H) == GLFW_RELEASE)
    {
        isOcclusionPressed = false;
    }
    if (glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS && !isLayersDifferPressed)
    {
        isLayersDiffer = !isLayersDiffer;
        isLayersDifferPressed = true;
    }
    if (glfwGetKey(window, GLFW_KEY_G) == GLFW_RELEASE)
    {
        isLayersDifferPressed = false;
    }
}
