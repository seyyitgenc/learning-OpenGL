...
int main() {
    ...
    Shader skyBoxShader("shaders/4.advanced_opengl/1.5.skybox.vs", "shaders/4.advanced_opengl/1.5.skybox.fs");
    Shader cubeShader("shaders/4.advanced_opengl/1.1.depth_testing.vs", "shaders/4.advanced_opengl/1.1.depth_testing.fs");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glm::mat4 projection = glm::perspective(glm::radians(camera.zoom), (float)SCR_WIDTH / (float)SCR_HEIGHTc, 0.1f, 100.0f);
...
unsigned int loadCubeMap(const std::vector<std::string>& faces) {
    ...
    for (size_t i = 0; i < faces.size(); i++) {
        ...
        if (data) {
            ...
            std::cout << "Cube map load failed at path : " << faces[i].c_str() << std::endl;
            stbi_image_free(data);
        }
    }
